...
import 'package:locacao/presentation/components/app_gri_categoria.dart';
...
class SelectCategoriaDialog extends StatefulWidget {
  ...
  final Function(Categorias) confirma;

  @override
  State<SelectCategoriaDialog> createState() => _SelectCategoriaDialogState();
}
        ...
        child: Column(
          ...
          children: [
            Padding(
              ...
              child: Text(widget.message, style: TextStyle(fontSize: 25)),
            ),
            Padding(
            ...
            FutureBuilder(
              future: Provider.of<CategoriasRepository>(context, listen: false).list(filter, 20, 1, []),
              builder: (context, snapshot) {
                if (snapshot.hasData) {
                  return Consumer<CategoriasRepository>(builder: (ctx, categorias, child) {
                    return CatGrid(
                      categorias: categorias.items,
                      onSelect: (clienteSel) {
                        categoria = clienteSel;
                      },
                    );
                  });
