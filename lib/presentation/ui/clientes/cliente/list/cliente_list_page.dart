...
class _ClienteListPageState extends State<ClienteListPage> {
  ...
  Future<void> _fetchData() async {
    ...
    await Provider.of<ClienteRepository>(context, listen: false).list(_query, _pageSize, _page, ['ASC', 'ASC']).then(
      (value) {
        clienteList = value;
        setState(() {
          _isLastPage = clienteList.length < _pageSize;
          _isLoading = false;
          _page = _page + 1;
          _cards.addAll(clienteList);
        });
      },
    ).catchError((error) {
      setState(() {
  ...
  void _hasErrorDialog() async {
    await showDialog(context: context, builder: (context) => AppPopErrorDialog(message: 'Ocorreu um erro ao carregar as clientes.')).then((value) {
      setState(() {
        _isLoading = true;
        _hasError = false;
        _fetchData();
                  ...
                  child: _cards.isEmpty
                      ...
                      : ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            final Cliente card = _cards[index];
                            return ClienteListWidget(card);
                          },
                        ),
                ),
              ),
