...
import '../../../../../domain/models/clientes/ativo_regras.dart';

class AtivoRegrasFormPage extends StatefulWidget {
  const AtivoRegrasFormPage({super.key});

  @override
  State<AtivoRegrasFormPage> createState() => _AtivoRegrasFormPageState();
}

class _AtivoRegrasFormPageState extends State<AtivoRegrasFormPage> {
  final _formKey = GlobalKey<FormState>();
  bool _dataIsLoaded = false;
  bool _isViewPage = false;
  final List<Widget> _regrasFields = [];
  List<String> _topicoText = [];
  final Map<Key, TextEditingController> _topicoTextControllers = {};
  final Map<Key, int> _fieldIndexes = {};

  final _controllers = AtivoRegraController(
  ...
  Widget build(BuildContext context) {
    ...
    return WillPopScope(
      onWillPop: () async {
        bool retorno = true;
        _isViewPage
            ? Navigator.of(context).pushNamedAndRemoveUntil('/ativo-regras-form', (route) => false)
            : await showDialog(
                context: context,
                builder: (context) {
      ...
      child: AppScaffold(
        title: Text('Adicione Regras ao Lugar'),
        showDrawer: false,
  ...
  Widget get _addRegraField {
    final fieldKey = UniqueKey();
    _fieldIndexes[fieldKey] = _regrasFields.length;
    ...
    return Container(
      ...
      child: IconButton(
        ...
        onPressed: () {
          setState(() {
            _regrasFields.add(_topicoField(fieldKey));
          });
        },
      ),
    );
  ...
  void _removeRegraField(Key fieldKey) {
    setState(() {
      final index = _fieldIndexes[fieldKey];
      _regrasFields.removeAt(index!);
  ...
  Future<void> _loadData(String id) async {
    await Provider.of<AtivoRegraRepository>(context, listen: false).get(id).then((ativoRegras) => _populateController(ativoRegras));
  }

  Future<void> _populateController(AtivoRegra ativoRegras) async {
    setState(() {
      _controllers.id!.text = ativoRegras.id ?? '';
      _controllers.ativoId!.text = ativoRegras.ativoId ?? '';
      _controllers.ativoNome!.text = ativoRegras.ativoNome ?? '';
      // _controllers.topico!.text = ativoRegras.topico ?? '';
    });
  }
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      await Provider.of<AtivoRegraRepository>(context, listen: false).save(payload, false).then((validado) {
        if (validado) {
          return showDialog(
            context: context,
            builder: (context) {
