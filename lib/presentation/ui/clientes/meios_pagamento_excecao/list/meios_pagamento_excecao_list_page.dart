...
import 'meios_pagamento_excecao_list_widget.dart';

class MeiosPagamentoExcecaoListPage extends StatefulWidget {
  const MeiosPagamentoExcecaoListPage({Key? key}) : super(key: key);

  @override
  State<MeiosPagamentoExcecaoListPage> createState() => _MeiosPagamentoExcecaoListPageState();
}

class _MeiosPagamentoExcecaoListPageState extends State<MeiosPagamentoExcecaoListPage> {
  String _query = '';
  late int _page;
  ...
  late List<MeiosPagamentoExcecao> _cards;
  ...
  Future<void> _fetchData() async {
    List<MeiosPagamentoExcecao> meiosPagamentoExcecaoList = [];
    await Provider.of<MeiosPagamentoExcecaoRepository>(context, listen: false)
        .list(_query, _pageSize, _page, ['ASC', 'ASC']).then(
      (value) {
        meiosPagamentoExcecaoList = value;
        setState(() {
          _isLastPage = meiosPagamentoExcecaoList.length < _pageSize;
          _isLoading = false;
          _page = _page + 1;
          _cards.addAll(meiosPagamentoExcecaoList);
        });
      },
    ).catchError((error) {
      setState(() {
  ...
  void _hasErrorDialog() async {
    await showDialog(
            context: context,
            builder: (context) => AppPopErrorDialog(message: 'Ocorreu um erro ao carregar as meio-pagamento-excecao.'))
        .then((value) {
      setState(() {
        _isLoading = true;
        _hasError = false;
  ...
  Widget build(BuildContext context) {
    ...
    return WillPopScope(
      ...
      child: AppScaffold(
        title: Text('Meios de Pagamento Excecao'),
        route: '/meio-pagamento-excecao-form',
        showDrawer: false,
        body: Center(
          child: Column(
                  ...
                  child: _cards.isEmpty
                      ...
                      : ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            final MeiosPagamentoExcecao card = _cards[index];
                            return MeiosPagamentoExcecaoListWidget(card);
                          },
                        ),
                ),
              ),
