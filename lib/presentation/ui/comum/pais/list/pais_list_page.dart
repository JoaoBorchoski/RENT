...
class _PaisListPageState extends State<PaisListPage> {
  ...
  Future<void> _fetchData() async {
    List<Pais> paisList = [];
    await Provider.of<PaisRepository>(context, listen: false).list(_query, _pageSize, _page, ['ASC', 'ASC']).then(
      (value) {
        paisList = value;
        setState(() {
  ...
  void _hasErrorDialog() async {
    await showDialog(
        context: context,
        builder: (context) => AppPopErrorDialog(message: 'Ocorreu um erro ao carregar as paises.')).then((value) {
      setState(() {
        _isLoading = true;
        _hasError = false;
        _fetchData();
