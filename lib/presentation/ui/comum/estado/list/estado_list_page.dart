...
class _EstadoListPageState extends State<EstadoListPage> {
  ...
  Future<void> _fetchData() async {
    ...
    await Provider.of<EstadoRepository>(context, listen: false).list(_query, _pageSize, _page, ['ASC', 'ASC']).then(
      (value) {
        estadoList = value;
        setState(() {
  ...
  void _hasErrorDialog() async {
    await showDialog(
        context: context,
        builder: (context) => AppPopErrorDialog(message: 'Ocorreu um erro ao carregar as estados.')).then((value) {
      setState(() {
        _isLoading = true;
        _hasError = false;
        _fetchData();
