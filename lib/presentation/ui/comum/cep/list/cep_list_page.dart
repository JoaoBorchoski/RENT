...
class _CepListPageState extends State<CepListPage> {
  ...
  Future<void> _fetchData() async {
    List<Cep> cepList = [];
    await Provider.of<CepRepository>(context, listen: false).list(_query, _pageSize, _page, ['ASC', 'ASC']).then(
      (value) {
        cepList = value;
        setState(() {
  ...
  void _hasErrorDialog() async {
    await showDialog(
        context: context,
        builder: (context) => AppPopErrorDialog(message: 'Ocorreu um erro ao carregar as ceps.')).then((value) {
      setState(() {
        _isLoading = true;
        _hasError = false;
        _fetchData();
