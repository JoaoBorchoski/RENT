...
class LocarListPage extends StatefulWidget {
  const LocarListPage({Key? key}) : super(key: key);

  @override
  State<LocarListPage> createState() => _LocarListPageState();
}

class _LocarListPageState extends State<LocarListPage> with SingleTickerProviderStateMixin {
  String _query = '';
  late int _page;
  ...
  Future<void> _fetchData() async {
    ...
    await Provider.of<AtivoRepository>(context, listen: false)
        .listComImg(_query, _pageSize, _page, ['ASC', 'ASC']).then(
      (value) {
        ativoList = value;
        setState(() {
  ...
  Future<void> _fetchDataCategoria() async {
    ...
    await Provider.of<CategoriasRepository>(context, listen: false)
        .listByClienteId(auth.usuarioAssociadoIdSelecionado, _query, _pageSize, _page, ['ASC', 'ASC']).then((value) {
      setState(
        () {
          _categorias = value;
        },
