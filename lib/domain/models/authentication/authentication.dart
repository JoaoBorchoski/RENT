...
import 'package:locacao/shared/config/app_menu_icons.dart';
...
class Authentication with ChangeNotifier {
  ...
  String? _userId;
  String? _tipoUser;
  String? _usuarioClienteId;
  String? _usuarioAssociadoIdSelecionado;
  ...
  String? get tipoUser => isAuth ? _tipoUser : null;

  set tipoUser(String? value) => _tipoUser = value;

  String? get usuarioClienteId => isAuth ? _usuarioClienteId : null;

  set usuarioClienteId(String? value) => _usuarioClienteId = value;

  String? get usuarioAssociadoIdSelecionado => isAuth ? _usuarioAssociadoIdSelecionado : null;

  set usuarioAssociadoIdSelecionado(String? value) => _usuarioAssociadoIdSelecionado = value;
  ...
  List<MenuData> getMenusOption() {
    List<MenuData> menu = [
      MenuData(
          Icons.home, _tipoUser == "Usuario" ? 'Locar' : 'Home', _tipoUser == "Usuario" ? '/locar' : '/home', false, '')
    ];
    ...
    _permissions?.forEach((menuOption) {
      ...
      for (var subMenuOption in subMenuOptions) {
        ...
        if ((_tipoUser == "Usuario") && (subMenuOption['text'] == "Locar")) {
        } else if ((_tipoUser == "Cliente Funcionario") && (subMenuOption['route'] == "/associado")) {
          menu.add(MenuData(
              indexIcon != -1 ? icons[indexIcon].icon : icons[0].icon, 'Associados', subMenuOption['route'], true, ''));
        } else if (([
          "/categoria",
          "/ativos",
          "/status-ativos",
          "/valor-convidado-nao-socio",
          "/meio-pagamento-aceito",
          "/meio-pagamento-excecao",
          "/cliente-preferencias",
          "/associado",
          "/status-associado"
        ].contains(subMenuOption['route']))) {
        } else {
          menu.add(MenuData(indexIcon != -1 ? icons[indexIcon].icon : icons[0].icon, subMenuOption['text'],
              subMenuOption['route'], true, ''));
        }
      }
    });
    menu.add(
